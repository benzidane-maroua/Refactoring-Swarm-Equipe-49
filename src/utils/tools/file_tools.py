...
def write_file(path: Path, content: str) -> None:
    ...
    """
    safe_path = ensure_safe_path(path) """ensures the path u passed is inside the sandbox"
...
def list_python_files(folder_path: Path) -> list[Path]:
    ...
    return [f for f in safe_folder.glob("*.py") if f.is_file()]  """ so u can loop through the files in the passed folder
